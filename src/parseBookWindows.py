...
for folder in folders:
    if os.path.isfile(folder + '/README.md'):
        with open(folder + '/README.md', "r", encoding='utf-8') as originalChapter:
            fileString = originalChapter.read()

            # Correct path for images
            imgPattern = r'(\!\[.*?\]\()(.*)'  # Two capturing groups: 1 for the markdown image syntax, 2 for the path
            subPattern = r'\g<1>{}/\g<2>'.format(re.escape(folder))  # Escape folder to avoid regex issues
...
for outputFormat in args.format:
    ...
    print("Running command:", pandocCommand)
    try:
        ...
        print(f"Error: {e}. Ensure 'pandoc' is installed and in PATH.")
